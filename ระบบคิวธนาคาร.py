...
class BankCustomer:
    def __init__(self, name, service_type, is_premium=False):
        ...
        self.queue_number = None  # กำหนดหมายเลขคิว (จะตั้งค่าใน BankQueue)
...
class BankQueue:
    ...
    def add_customer(self, customer):
        ...
        customer.queue_number = self.waiting_count  # ตั้งหมายเลขคิว
    ...
    def serve_next_customer(self):
        ...
        print(f"\nเรียกลูกค้า: {customer.name} (คิวที่ {customer.queue_number})")
        print(f"บริการ: {customer.service_type}")
    ...
    def display_queue(self):
        ...
        while temp_queue:
            ...
            print(f"{position}. {customer.name} - {customer.service_type} (คิวที่ {customer.queue_number})")
            position += 1
        print("-" * 30)
